...
float AU_to_m = 1.49597870691e11;
...
void setup() {
  ...
  //Load TNO data
  //String[] lines = loadStrings("tno_formatted.dat");
  String[] lines = loadStrings("Distant.txt");
  String[] rad_lines = loadStrings("tno_formatted.dat");
  String[] planet_lines = loadStrings("planet_orbital.dat");
  ...
  float[] a_arr_temp = new float[nstart]; //semi major axis, m
  float[] e_arr_temp = new float[nstart]; //eccentricity
  float[] lilomega_arr_temp = new float[nstart]; //argument of periapsis [radians]
  float[] bigomega_arr_temp = new float[nstart]; //longitude of ascending node [radians]
  float[] inc_arr_temp = new float[nstart]; //inclination [radians]
  float[] M0_arr_temp = new float[nstart]; //Mean anomaly
  float[] r_arr_temp = new float[nstart]; //Diameter [km]
  ...
  for (int ii = 0; ii < lines.length; ii++) {
    ...
    //Only include if radius above minimum
...
float E_of_t(float Mt, float e)
{
  ...
  while (current_precision > desired_precision) {
    ...
    /*
    println("wta = ", (Et - e*sin(Et) - Mt));
    println("Et = ", Et);
    println("wtb = ", (1.0 - e*cos(Et)));
...
void draw() {
  ...
  if (do_moving_view) {
    ...
    rotateY(mx/float(width)* PI - PI/4 + 0.2);
    perspective(fov, aspect, cameraZ/10.0, cameraZ*10.0);
  }
  /*
  if (do_moving_view){
   float cameraY = height/2.0;
   float fov = mx/float(width) * PI/2;
   float cameraZ = cameraY / tan(fov / 2.0);
   float aspect = float(width)/float(height);
   //perspective(fov, aspect, cameraZ/10.0, cameraZ*10.0);
   //translate(centerx + width/2, centery + height/2, -100);
   //rotateY(-PI/2);
   //rotateY(PI/3 + my/float(height) * PI);
   //rotateY( my/float(height) * PI);
   }
   */
